...
if($password!=$confirm)
{
	header("location:donarreg.php?reg=fail");
}else{
$newpass=md5($password);
if(((in_array($filecheck1,$fileextstored1)))){
	$id_proof='upload/'.$filename1;
	move_uploaded_file($filetemp1,$id_proof);
$sel=mysqli_query($con,"INSERT INTO `donar` (`email`, `username`, `address`, `password`,`phone`,`photo`,`id_proof`) VALUES ('$email', '$name', '$address', '$newpass','$phone','$photo','$id_proof')") or die(mysqli_error($con)); 
header("location:donarreg.php?reg=success");
}
}
?>
